...
# 指定客户端导出使用的 TLS fingerprint 和 ALPN
...
- 默认分享链接带 `fp=chrome`；可通过 `--fingerprint firefox --alpn h2,http/1.1` 调整 Fingerprint 和 ALPN。
...
- `v2rayn-share.txt`：给 v2RayN 导入的专用 `v2rayn://anytls/<base64url-json>` 链接，格式借鉴 fscarmen/sing-box。
- `v2rayn-insecure-share.txt`：给 v2RayN 诊断证书校验问题的同格式链接，`AllowInsecure` 为 `true`；确认问题后应优先修客户端信任链。
...
sudo cat /etc/anytls/exports/v2rayn-share.txt
```

v2RayN 优先导入 `v2rayn-share.txt`，不要再把通用 `anytls://...` 当作 v2RayN 主导入方式。Clash Verge Rev 可使用 `clash-verge.yaml` 或通用 `share-link.txt`。
...
- 默认 ALPN 为空，fingerprint 为 `chrome`；可通过 `--alpn` 和 `--fingerprint` 显式设置。
